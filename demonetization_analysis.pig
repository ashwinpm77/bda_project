raw_data = load '/demon_analysis/demonetization-tweets.csv' using PigStorage(',');
...
dictionary = load '/demon_analysis/AFINN.txt' using PigStorage('\t') as (word:chararray,rating:int);
...
store positive_tweets INTO '/demon_analysis/positive_tweets' using PigStorage(',');

store negative_tweets INTO '/demon_analysis/negative_tweets' using PigStorage(',');
